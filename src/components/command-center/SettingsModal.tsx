...
export function SettingsModal({
  ...
  }, [edition.loading, edition.selfHosted, isOpen, refreshBillingStatus]);
  ...
  const handleResendInvite = async (invite: WorkspaceInvite) => {
    const actionKey = `resend-invite:${invite.id}`;
    setTeamActionLoading(actionKey);
    setInviteError(null);
    try {
      const res = await fetch("/api/workspaces/invites", {
        method: "PATCH",
        ...
        setInvites((current) =>
          current.map((entry) =>
            entry.id === invite.id
              ? {
                  ...entry,
                  token: String(json.invite.token),
                  expires_at: String(
                    json.invite.expires_at || entry.expires_at,
                  ),
                }
              : entry,
          ),
      ...
      setSuccessMessage(`Invitation email sent again to ${invite.email}.`);
      setSuccess(true);
                    ...
                    <div key={inv.id} className="flex items-center justify-between gap-2 py-1">
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        <button
                          type="button"
                          disabled={
                            teamActionLoading === `resend-invite:${inv.id}`
                          }
                          onClick={() => void handleResendInvite(inv)}
                          className="px-2 py-1 rounded-md bg-cyan-500/10 border border-cyan-500/20 text-cyan-200 hover:bg-cyan-500/15 text-xs inline-flex items-center gap-1 disabled:opacity-40"
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded-md bg-white/5 border border-white/10 text-zinc-200 hover:bg-white/10 text-xs inline-flex items-center gap-1"
...
{REPAIR_CONNECTOR_COMMANDS}
  ...
  const activeSectionMeta =
    SECTION_NAV.find((section) => section.id === activeSection) ||
    SECTION_NAV[0];
  const visibleSectionNav = SECTION_NAV.filter(
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-50 flex h-[100dvh] flex-col bg-[#0a0a0f]"
      >
        {/* Header */}
        <div className="flex min-h-14 shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-[#0a0a0f] px-4 pb-3 pt-[calc(.75rem+env(safe-area-inset-top))] sm:px-6 sm:py-3">
          ...
          <div className="flex items-center gap-2">
            <a
              href="/settings"
              className="hidden items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-xs text-zinc-400 transition hover:border-white/20 hover:text-white sm:flex"
            >
              Workspace settings
              <ExternalLink className="h-3.5 w-3.5" />
            </a>
            <button
              onClick={onClose}
              className="flex h-9 w-9 items-center justify-center rounded-lg text-zinc-500 transition-all hover:bg-white/10 hover:text-white"
              aria-label="Close Command Center settings"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col md:flex-row">
          <div className="shrink-0 border-b border-white/10 bg-black/10 p-3 md:hidden">
            <CustomSelect
              value={activeSection}
              onChange={(nextSection) =>
                setActiveSection(nextSection as SettingsSection)
              }
              options={visibleSectionNav.map((section) => ({
                value: section.id,
                label: section.label,
              }))}
              size="sm"
              ariaLabel="Command Center settings section"
              triggerClassName="bg-white/[0.025]"
            />
            <a
              href="/settings"
              className="mt-2 flex w-full items-center justify-between rounded-lg border border-white/[0.07] bg-white/[0.02] px-3 py-2 text-xs text-zinc-500"
            >
              Open shared workspace settings
              <ExternalLink className="h-3.5 w-3.5" />
            </a>
          </div>
          {/* Left sidebar */}
          <nav className="hidden w-60 shrink-0 overflow-y-auto border-r border-white/10 px-3 py-4 md:block">
            <a
              href="/settings"
              className="mb-4 flex items-center justify-between rounded-xl border border-cyan-400/15 bg-cyan-400/[0.035] px-3 py-2.5 text-xs text-zinc-300 transition hover:border-cyan-400/25 hover:text-white"
            >
              Shared workspace settings
              <ExternalLink className="h-3.5 w-3.5 text-cyan-300" />
            </a>
            <div className="px-3 pb-2 text-[9px] font-medium uppercase tracking-[0.18em] text-zinc-600">
              Command Center
            </div>
              ...
              {visibleSectionNav.map((section) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full shrink-0 items-center gap-2.5 whitespace-nowrap rounded-lg px-3 py-2.5 text-left text-sm transition-all ${
                      isActive
